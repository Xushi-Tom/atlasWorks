...
from terrainOps import createTerrainTiles
...
def _handle_shutdown(signum, frame):
    global _RUNNING
    _RUNNING = False
    logMessage(f"地形切片 worker 收到退出信号: {signum}", "INFO")
...
def _dispatch_job(job):
    ...
    try:
        if job_type in {"terrain_tiles", "terrain"}:
            with _APP.test_request_context(json=_worker_payload(job)):
                createTerrainTiles()
            return True
    except Exception as exc:
        logMessage(f"地形切片 worker 执行任务失败: {job.get('taskId')} - {exc}", "ERROR")
        return False

    logMessage(f"地形切片 worker 当前尚未接入该任务类型: {job_type}", "WARNING")
    return False


def main():
    worker_config = config.get("worker", {})
    worker_id = str(worker_config.get("id") or "atlasworks-terrain-worker")
    ...
    logMessage(f"AtlasWorks 地形切片 worker 启动: {worker_id}, jobTypes={','.join(job_types) or '*'}", "INFO")
    ...
    while _RUNNING:
        ...
        logMessage(f"地形切片 worker 领取任务: {job.get('taskId')} ({job.get('jobType')})", "INFO")
        _dispatch_job(job)
        time.sleep(poll_interval)
