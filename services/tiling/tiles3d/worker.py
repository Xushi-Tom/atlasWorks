...
def _handle_shutdown(signum, frame):
    global _RUNNING
    _RUNNING = False
    logMessage(f"3D Tiles worker 收到退出信号: {signum}", "INFO")
...
def _dispatch_job(job):
    ...
    try:
        if job_type in {"3dtiles", "3dtiles-pointcloud", "3dtiles-vector", "3dtiles-model", "3dtiles-osgb"}:
            with _APP.test_request_context(json=_worker_payload(job)):
                create3DTiles()
            return True
    except Exception as exc:
        logMessage(f"3D Tiles worker 执行任务失败: {job.get('taskId')} - {exc}", "ERROR")
        return False

    logMessage(f"3D Tiles worker 当前尚未接入该任务类型: {job_type}", "WARNING")
    return False


def main():
    worker_config = config.get("worker", {})
    worker_id = str(worker_config.get("id") or "atlasworks-3dtiles-worker")
    ...
    logMessage(f"AtlasWorks 3D Tiles worker 启动: {worker_id}, jobTypes={','.join(job_types) or '*'}", "INFO")
    ...
    while _RUNNING:
        ...
        logMessage(f"3D Tiles worker 领取任务: {job.get('taskId')} ({job.get('jobType')})", "INFO")
        _dispatch_job(job)
        time.sleep(poll_interval)
