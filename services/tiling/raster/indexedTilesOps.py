...
def cleanupTileGridIndexArtifacts(outputPath):
    removedFiles = []
    artifactNames = [
        "tile_index.geojson",
        "tile_index.shp",
    ...
    for artifactName in artifactNames:
        artifactPath = os.path.join(outputPath, artifactName)
        if not os.path.exists(artifactPath):
            continue
        try:
            os.remove(artifactPath)
            removedFiles.append(artifactPath)
...
def generateShapefileIndex(tileIndex, outputPath, generateShp=True):
    try:
        ...
        if not generateShp:
            removedFiles = cleanupTileGridIndexArtifacts(outputPath)
            if removedFiles:
                logMessage(f"已禁用网格文件输出，清理旧索引文件 {len(removedFiles)} 个")
            else:
                logMessage("已禁用网格文件输出，跳过 tile_index 文件生成")
            return {
                "success": True,
                "shpFile": None,
                "geojsonFile": None,
...
def createIndexedTilesLegacy():
    try:
        ...
        outputPath, outputPathArray, outputPathAutoGenerated = resolveTilesOutputPath(outputPathArray, "map")
        os.makedirs(outputPath, exist_ok=True)
        if outputPathAutoGenerated:
            logMessage(f"未传 outputPath，已自动生成地图输出目录: {outputPath}")
        ...
        def finalize_from_existing_metadata(existingMetadata):
            if not generateShpIndex:
                removedFiles = cleanupTileGridIndexArtifacts(outputPath)
                if removedFiles:
                    logMessage(f"增量复用前已清理旧网格文件 {len(removedFiles)} 个")
...
def _write_geoserver_tile(tile_info, output_path, workspace, layer_names, image_format, tile_scheme, tile_size, transparent_background=True):
    zoom = int(tile_info["z"])
    tile_x = int(tile_info["x"])
    tile_y = int(tile_info["y"])
    tile_file, _, _ = getTileOutputPath(
        output_path,
        zoom,
        tile_x,
        tile_y,
    ...
    if response.status_code != 200 or not content_type.startswith("image/"):
        ...
        raise RuntimeError(f"GeoServer WMS 出图失败 z={zoom} x={tile_x} y={tile_y}: {detail}")
    with open(temp_file, "wb") as file_obj:
        file_obj.write(response.content)
    os.replace(temp_file, tile_file)
    return {"success": True, "tileFile": tile_file}
...
def processIndexedTilesWithGeoserver(tif_files, output_path, output_path_array, task_id, render_options, min_zoom, max_zoom, tile_size, image_format, tile_scheme, enable_incremental_update=False):
    ...
    try:
        ...
        def process_tile(tile_info):
            try:
                ...
                return {"success": True, "tile": tile_info}
            except Exception as exc:
                return {"success": False, "tile": tile_info, "error": str(exc)}

        with ThreadPoolExecutor(max_workers=wms_concurrency) as executor:
            future_map = {executor.submit(process_tile, tile_info): tile_info for tile_info in tile_index}
            for index, future in enumerate(as_completed(future_map), start=1):
                ...
                if index == total_tiles or index % 20 == 0:
                    progress = 10 + int((index / max(1, total_tiles)) * 85)
                    elapsed = max(0.001, time.time() - start_time)
                    speed = round(index / elapsed, 2)
                    with taskLock:
                        current = taskStatus.get(task_id)
                        if current:
                            current["progress"] = min(99, progress)
                            current["currentStage"] = "GeoServer 出图切片"
                            current["message"] = f"GeoServer 切片处理中 {index}/{total_tiles}，失败 {failed_tiles}"
                            current["stats"] = {
                                "totalTiles": total_tiles,
                                "processedTiles": processed_tiles,
                                "failedTiles": failed_tiles,
                                "remainingTiles": max(0, total_tiles - processed_tiles - failed_tiles),
                                "averageSpeed": speed,
