    ...
    try:
        ...
        system_info = {
            ...
            "config": {
                ...
                "geoserverBaseUrl": config.get("geoserverBaseUrl") or "",
                "geoserverPublicBaseUrl": config.get("geoserverPublicBaseUrl") or "",
                "geoserverWorkspace": config.get("geoserverWorkspace") or "",
...
def listApiRoutes():
    try:
        ...
        routes = [
            ...
            {"path": "/api/geoserver/health", "methods": ["GET"], "description": "GeoServer 健康检查", "category": "发布管理", "logic": "检查 GeoServer REST 服务连通性与版本信息"},
            {"path": "/api/geoserver/publish", "methods": ["POST"], "description": "发布影像到 GeoServer", "category": "发布管理", "logic": "将单个 GeoTIFF 或目录 ImageMosaic 发布为 GeoServer 图层，可选触发 GeoWebCache 预切片"},
            {"path": "/api/geoserver/layers", "methods": ["GET"], "description": "列出 GeoServer 图层", "category": "发布管理", "logic": "返回指定 workspace 下的 GeoServer 图层列表"},
            {"path": "/api/geoserver/layers/<name>", "methods": ["GET", "DELETE"], "description": "GeoServer 图层详情/删除", "category": "发布管理", "logic": "读取 GeoServer 图层详情，或递归删除对应 store 和图层"},
            {"path": "/api/geoserver/layers/<name>/seed", "methods": ["POST"], "description": "GeoServer 预切片", "category": "发布管理", "logic": "触发 GeoWebCache 对指定 GeoServer 图层执行 seed 预切片"},
            ...
            {"path": "/publication-assets/<publicationId>/<path:relative_path>", "methods": ["GET"], "description": "访问指定发布记录资源", "category": "发布管理", "logic": "按 publicationId 做轻量路径换算后重定向到静态后端，兼容 XYZ/TMS"},
            {"path": "/published", "methods": ["GET"], "description": "浏览发布根目录", "category": "发布管理", "logic": "将发布目录入口重定向到静态后端"},
            {"path": "/published/<path:relative_path>", "methods": ["GET"], "description": "访问发布资源", "category": "发布管理", "logic": "将已发布目录资源重定向到静态后端"},
